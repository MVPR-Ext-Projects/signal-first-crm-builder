...
export function CampaignEnrollChip({
  workspaceId,
  contactId,
}: {
  workspaceId: string
  ...
  const [busy, setBusy]           = useState<string | null>(null) // campaignId being toggled
  ...
  async function fetchCampaigns() {
    ...
    try {
      const res = await fetch(
        `/api/dashboard/${workspaceId}/contacts/${contactId}/campaigns`,
      )
  ...
  async function toggle(campaign: Campaign) {
    ...
    try {
      if (campaign.enrolled) {
        await fetch(
          `/api/dashboard/${workspaceId}/contacts/${contactId}/campaigns/${campaign.id}`,
          { method: "DELETE" },
        )
      } else {
        await fetch(
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      <button
        type="button"
        title={enrolledCount > 0 ? `In ${enrolledCount} campaign${enrolledCount === 1 ? "" : "s"}` : "Add to campaign"}
        onClick={handleOpen}
        className={`relative flex h-7 w-7 items-center justify-center rounded-[7px] bg-white/[0.06] transition-colors hover:bg-white/[0.12] motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2BA98B]/60 ${open ? "bg-white/[0.12]" : ""}`}
      >
          ...
          >
            <div className="flex items-center gap-1.5">
              <input
                ...
                disabled={creating}
                className="flex-1 min-w-0 rounded-md border border-white/10 bg-white/[0.04] px-2.5 py-1.5 text-[12px] text-zinc-200 placeholder-zinc-600 focus:border-[#2BA98B]/40 focus:outline-none focus:ring-1 focus:ring-[#2BA98B]/40 disabled:opacity-50"
              />
              <button
                type="submit"
